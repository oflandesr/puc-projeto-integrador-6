...
public class WalletQueriedImpRepository implements WalletQueriedRepository {
	...
	public List<VariableTransactionDto> findWalletVariableTransaction(Long walletId, LocalDate startDate,
		...
		String nativeQuery = """
				    SELECT
					    TICKER_ID,
					    SUM(CASE WHEN BUY_OR_SALE = 0 THEN AMOUNT ELSE 0 END) AS TOTAL_AMOUNT_BUYED,
					    SUM(CASE WHEN BUY_OR_SALE = 1 THEN AMOUNT ELSE 0 END) AS TOTAL_AMOUNT_SOLD,
					    AVG(CASE WHEN BUY_OR_SALE = 0 THEN PRICE ELSE NULL END) AS AVG_VALUE_INVESTED
					FROM
