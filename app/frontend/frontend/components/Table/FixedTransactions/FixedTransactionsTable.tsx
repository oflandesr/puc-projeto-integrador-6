...
import { getFixedTransactionsTableCols, tableCols } from "@/mock/mock";
...
const FixedTransactionsTable = ({ customColSpan, customRowSpan, parentRefresh}: WalletsTableProps) => {
    ...
    async function handleDeleteTransaction(waletId: string, transactionId: string) {
        try {
            const url = `/wallet/${walletId}/tfixed/${transactionId}`;
            const headers = {
                username: getUserData().login,
                password: getUserPassword(),
            };
    ...
    return (
        <>
            {walletData ? (
                <Table columns={getFixedTransactionsTableCols(walletId, handleDeleteTransaction)} data={walletData.fixedIncomeTransactions} />
            ) : (
...
export default FixedTransactionsTable;
